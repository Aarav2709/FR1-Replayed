...
function scene:createScene(event)
    ...
    ---------------------------------------------------------------------------
    createSpriteModule.updateAvatar(storyboard.database.getAvatarData())
...
function scene:enterScene(event)
    ...
    local function updateNotificationBadge()
        ...
        if count > 0 then
            ...
            if socialAlertImage then socialAlertImage:removeSelf(); socialAlertImage = nil end
            if socialAlertText then socialAlertText:removeSelf(); socialAlertText = nil end
        end
    end
    ...
    local function updateMarketBadge()
        ...
        if count > 0 then
            ...
            if marketAlertImage then marketAlertImage:removeSelf(); marketAlertImage = nil end
            ...
            if marketAlertText then marketAlertText:removeSelf(); marketAlertText = nil end
...
function scene:exitScene(event)
    ...
    if marketAlertImage then marketAlertImage:removeSelf(); marketAlertImage = nil end
    if marketAlertText then marketAlertText:removeSelf(); marketAlertText = nil end
    if alertRef then native.cancelAlert(alertRef); alertRef = nil end
    if delayTimer then timer.cancel(delayTimer); delayTimer = nil end
    if loadingText then loadingText:removeSelf(); loadingText = nil end
    if loadingBackground then loadingBackground:removeSelf(); loadingBackground = nil end
    if videoHideTimer then timer.cancel(videoHideTimer); videoHideTimer = nil end
