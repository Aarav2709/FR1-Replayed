...
--------------------------------------------------------------------------------
-- Initialize Default Player Data (first run)
--------------------------------------------------------------------------------

function database.initializeDefaultData()
  -- Check if avatar data exists
  local existingAvatar = database.getAvatarData()
  if existingAvatar then
    -- Already initialized
    return
  end

  -- Set default avatar: Fox (100), no hat (200), no item (300), no boots (400)
  database.setAvatarData({ 100, 200, 300, 400 })

  -- Set starting items (Fox is free, so add it to owned)
  storyboard.databaseData.items = { { 100 }, {}, {}, {} }

  -- Money already set in main.lua via setMoney(10000000)
end
